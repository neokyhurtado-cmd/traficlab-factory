# CARRIER.md — HERMES-VERIFY-G5

```text
ROLE                 = HERMES-VERIFY-G5
WORKTREE             = .worktrees/wt-orca-g5-visor-verify
BRANCH               = feat/g5-visor-verify-sha37
...
Per-video runtime/visor verification of HERMES-PROCESS-G5 outputs. Loads the visor locally, plays each P5/P6/P7 video, verifies track counts, IDOM mapping, hero/identity bindings, and IDOM-counter display correctness. Reports per-video PASS/FAIL with evidence.

## HARD STOPS

NO bulk reruns. NO GPU contention. Read-only against outputs; writes only to swarm/VERIFY_REPORT.md.

## HANDOFF

HERMES-REVIEW-G5 (per-video verdict table).

## DELIVERABLE

swarm/VERIFY_REPORT.md (per-video table + summary) inside this worktree (committed).

...
from jev_shadow.provider import ProviderResult, TypeSafeJevProvider
...
class FakeProvider:
    ...
    def evaluate(self, *, state, questions):
        ...
        return ProviderResult("OK", self.answers, {"model": "jev-test"})
...
def test_shadow_decision_normalizes_sdk_answer_shapes():
    decision = evaluate_shadow(
        {"task_id": "t3"},
        provider=FakeProvider(fake_answers()),
    )
...
def test_provider_without_key_is_fail_soft(monkeypatch):
    monkeypatch.delenv("TYPESAFE_API_KEY", raising=False)
    result = TypeSafeJevProvider().evaluate(
        state={"task_id": "t4"},
        questions=jev_questions(),
    )
    assert result.status == "UNAVAILABLE"
    assert "TYPESAFE_API_KEY" in (result.error or "")


def test_runtime_writes_evidence_without_key(tmp_path, monkeypatch):
    monkeypatch.delenv("TYPESAFE_API_KEY", raising=False)
    directive = SimpleNamespace(
        directive_id="JEV-TEST-1",
        scope="read-only test",
        repository="traficlab-factory",
        target_branch="main",
        action="TEST",
        requires_human_go_real=False,
    )
    decision = record_directive_shadow_decision(directive, str(tmp_path))
    assert decision["provider_status"] == "UNAVAILABLE"
    stored = json.loads((tmp_path / "jev_shadow.json").read_text(encoding="utf-8"))
    assert stored["may_control_execution"] is False
